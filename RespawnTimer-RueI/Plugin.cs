...
public class RespawnTimer : Plugin<Config>
{
    ...
    private void DownloadDefaultTimer(string defaultTimerDirectory)
    {
        ...
        LogManager.Info("Downloading DefaultTimer-RueI.zip...");
        var url = $"https://github.com/MedveMarci/RespawnTimer/releases/download/{Version}/DefaultTimer-RueI.zip";
        try
        {
        ...
        catch (WebException e)
        {
            if (e.Response is HttpWebResponse response)
                LogManager.Error(
                    $"Error while downloading DefaultTimer-RueI.zip: {(int)response.StatusCode} {response.StatusCode}");
            return;
        }

        LogManager.Info("DefaultTimer-RueI.zip has been downloaded!");
