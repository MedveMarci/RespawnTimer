...
{
    private static readonly string TimerBeforeSpawn =
        "<size=50%>Round time: {round_minutes}:{round_seconds} | TPS: {tps}/{tickrate}</size>"
        + new string('\n', 18)
        + "<size=75%><color={RANDOM_COLOR}>You will respawn in: </color>\n"
    ...
    private static readonly string TimerDuringSpawn =
        "<size=50%>Round time: {round_minutes}:{round_seconds} | TPS: {tps}/{tickrate}</size>"
        + new string('\n', 18)
        + "<size=110%><color={RANDOM_COLOR}>You will spawn as:</color> {team} {sseconds} s\n"
