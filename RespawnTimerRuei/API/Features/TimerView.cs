namespace RespawnTimerRuei.API.Features
{
    ...
    public partial class TimerView
    {
        ...
        public static void AddTimer(string name)
        {
            ...
            var directoryPath = Path.Combine(RespawnTimerRuei.RespawnTimerDirectoryPath, name);
        ...
        public static bool TryGetTimerForPlayer(Player player, out TimerView timerView)
        {
            ...
            // Check by group name
            if (groupName is not null && RespawnTimerRuei.Singleton.Config.Timers.TryGetValue(groupName, out var timerName))
            {
            ...
            // Check by user id
            if (RespawnTimerRuei.Singleton.Config.Timers.TryGetValue(player.UserId, out timerName))
            {
                timerView = CachedTimers[timerName];
                return true;
            }
            ...
            if (RespawnTimerRuei.Singleton.Config.Timers.TryGetValue("default", out timerName))
            {
                timerView = CachedTimers[timerName];
                return true;
            }
